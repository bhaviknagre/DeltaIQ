...
def client():
    ...
    register_pid("26-9026-REV-A", "data/samples/pair_native/rev_a.pdf", "Rev A")
    register_pid("26-9026-REV-B", "data/samples/pair_native/rev_b.pdf", "Rev B")
...
def test_results_page_shows_criticality_signal(client):
    resp = client.get("/results", params={"pid_a": "26-9026-REV-A", "pid_b": "26-9026-REV-B"})
    assert resp.status_code == 200
...
def test_report_json_matches_delta_engine(client):
    resp = client.get("/report/json", params={"pid_a": "26-9026-REV-A", "pid_b": "26-9026-REV-B"})
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["pid_a"] == "26-9026-REV-A"
...
def test_chat_api_returns_grounded_citation(client):
    resp = client.post(
        "/api/chat",
        json={"pid_a": "26-9026-REV-A", "pid_b": "26-9026-REV-B", "question": "What changed with tag 26-KA-902?"},
    )
    assert resp.status_code == 200
    payload = resp.json()
